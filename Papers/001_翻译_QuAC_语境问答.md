...
​		有趣的是，选段中间位置的文本被问到的频率不是很高（图5c）。当更多问题被问到，问题就有更大的可能是没有回答的。
...
​		我们考虑下面的QA任务：给定最开始的k个问题，和它们在对话中真实的回答、所有的支持材料（实体e、话题t、背景b、选段s）（ps: supporting material，能理解，翻译后挺奇怪的）和问题q_(k+1)，我们预测回答，回答在选段s中索引为i, j。

​		因为问题的确认在没有对错（yes/no）回答下是不完整的，并且连续的反馈对于“信息获取”对话是非常重要的，所以我们预测了对话指令v，并伴随着来自最后答案预测a_(k+1)。
...
​		对于“对话指令”，我们对大多数的注解给出了准确率，并随机断开联系。
...
​		多数回答是“没有回答no anwer”和其它对话指令的多数类别（neither用来确认，dont't follow up就是继续）。
...
​		表4总结了我们的结果（每个单元格显示了一个dev/test分数），其中“对话指令”是Yes/No（表示确认）和Follow up（继续）。
